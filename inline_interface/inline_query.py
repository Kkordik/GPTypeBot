...
from GPTBot.texts import example_queries
from GPTBot.classes.MainClasses import User, QueryDb
from GPTBot.database.run_db import user_tb, query_tb
from GPTBot.classes.Markers import ends_with_marker
from GPTBot.classes.Tip import NoSubscription, StartWithMarker, TooLongQuery, MsgAnswerMistake, EndWithSign, InfoTip,\
    AnswerTip, TimeLimitTip
from GPTBot.config import TELEGRAM_CHAR_LIMIT, END_TIP_PROBABILITY, INLINE_DEF_TOKEN_NUM, INLINE_MAX_TOKEN_NUM,\
...
async def inline_echo(inline_query: InlineQuery):
    ...
    if "get_by_id." in inline_query.query:
        param = inline_query.query.split("get_by_id.")[1]
        ...
        try:
            ...
            await MsgAnswerMistake(language=user_db.language).send_inline_tip(inline_query=inline_query,
                                                                              result_id=result_id,
                                                                              user_db=user_db)
            print(datetime.datetime.now(), ex, sep="  inline_get_by_id  ")
